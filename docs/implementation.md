...
## 45. TOTP 秘密鍵欠落時の自動無効化（2025-11-04）
- [x] ログイン時に `is_totp_enabled=1` かつ `totp_secret` が未設定の場合、自動的に TOTP を無効化しログイン可能とした。
- [x] `set_totp_status()` で秘密鍵未設定状態のまま有効化しようとするとエラーを返すようにした。
