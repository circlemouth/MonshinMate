...
- **備考**: LLM が無効化されている場合は常に空配列を返す。
  - リモート LLM との通信に失敗しスタブへフォールバックした場合、レスポンスヘッダ `X-LLM-Error` にエラー内容が格納される。
...
## POST /llm/chat
- **概要**: LLM と対話し簡易応答を取得する。
- **リクエストボディ**:
  - `message` (str): ユーザーからのメッセージ
- **レスポンス**:
  - `reply` (str): 応答文字列。リモート失敗時はスタブ応答となる。
  - リモート通信に失敗した場合、レスポンスヘッダ `X-LLM-Error` にエラー内容が含まれる。
...
- **レスポンス**:
  ...
  - 通信に失敗した場合、レスポンスヘッダ `X-LLM-Error` に詳細が格納される。
