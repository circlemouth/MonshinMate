...
5. `ENABLE_GCP=1`（または `MONSHINMATE_ENABLE_GCP_LLM=true`）の環境でプライベートサブモジュールが GCP 用 LLM アダプタを提供している場合のみ、Vertex AI などのクラウドプロバイダが選択肢に追加されます。追加されたプロバイダでは、プロジェクトIDやロケーションなど必要な設定フィールドが自動的に表示されます。
