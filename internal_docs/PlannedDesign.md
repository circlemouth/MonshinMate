...
- **表示形式**（採用）
  - **複数カード**を縦に提示し、まとめて回答→送信（上限N件）
  - 1問ずつ表示は採用しない（将来のオプション化は可）
- **ヘッダー**：`追加で確認をさせていただきます。分かる範囲でお答えください。`（説明：不足/曖昧な点のみを最小限質問します）
...
## 16. 文言・マイクロコピー（例）
- 追加質問画面タイトル：`追加で確認をさせていただきます。分かる範囲でお答えください。`
