...
> **注意**: SQLite データベース（Docker Compose 既定: `./data/sqlite/app.sqlite3`）は初回起動時に自動生成されます。リポジトリには同梱されていないため、旧バージョンの DB を流用しない場合は起動前に既存ファイルを削除しておいてください。旧構成で `backend/app/app.sqlite3` を使用している場合は、ファイルを `./data/sqlite/` に移動するか `MONSHINMATE_DB` を旧パスに設定してください。
...
- 仕様:
  ...
  - 入力: `emergency_password`（環境変数に設定した値）と `new_password`（8文字以上）
  - 条件: 管理者の TOTP が無効（`/admin/totp/mode` が `off`）であること
  - 成功時: パスワード更新後、TOTP は無効化（秘密鍵クリア）されます。必要に応じて再設定してください。
- 推奨運用:
  - `ADMIN_EMERGENCY_RESET_PASSWORD` は十分に強度の高い値を設定し、サーバ上でのみ管理してください。
  - 可能であればリバースプロキシやファイアウォールで管理ネットワーク限定のアクセス制御を併用してください。
   - セキュリティ画面の「パスワードの変更」から現在のパスワードを入力して更新できます。変更すると二段階認証は自動的に無効化されるため、必要に応じて再設定してください。

## 2. システム表示名の設定
1. 左メニューから「システム表示名」を選択します。
2. 表示されたフォームにクリニック名など任意の表示名を入力すると、数秒以内に自動保存されます。
3. 保存操作は不要で、患者向け画面のヘッダーに即時反映されます。

## 3. テーマカラーの設定
1. 左メニューから「テーマカラー」を選択します。
2. 表示されたパステルカラー一覧から選ぶか、カラーコードを入力して任意の色を指定します。
3. 選択・入力したカラーは自動的に保存され、全画面に即時反映されます。
...
4. モデル名の扱いはプロバイダによって異なります。
   - `ollama` / `lm_studio` / `openai` では「使用可能なモデル一覧を取得」ボタンで候補を読み込み、プルダウンから選択できます。保存済みのモデルは一覧取得前でも選択欄に表示されます（内部的には `provider` と追加フィールドを `/llm/list-models` へ送信）。
   - `gcp_vertex`（Google Cloud Vertex AI）の場合は Google 側で一覧取得 API が制限されているためボタン・プルダウンは表示されません。モデル名を直接入力し、「疎通テストを実行」で接続可否を確認してください。
   - `gcp_vertex` の認証にはApplication Default Credentialsを使います。Cloud Runでは実行サービスアカウントを割り当て、JSONキーファイルを環境変数、管理画面、Firestoreへ保存しないでください。
5. 「保存」を押すと自動で疎通テストが実行されます。失敗した場合はポップアップでエラーメッセージが表示されます。
6. テンプレート側の「追質問を有効化」チェックは、LLM設定が有効かつ疎通テストに成功している場合のみオンにできます。
7. 画面上部のバーとダッシュボードのカードで接続状況を確認できます（緑=疎通良好、黄=確認待ち/無効、赤=エラー）。
   - 状態は管理者ログインや疎通テスト、追加質問生成・サマリー生成などの LLM 呼び出しが完了したタイミングで更新されます。
   - 管理者JWT付きのREST API `/system/llm-status` からも最新スナップショット（状態、最終更新時刻、更新契機）を取得できます。患者画面は状態を別途照会せず、実際の問診処理が失敗した場合も既定質問へフォールバックして完了まで進みます。
