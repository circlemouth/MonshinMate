...
## 112. PDF出力レイアウト刷新（完了）
- 目的: 管理画面から出力する問診票PDFを紙面運用に近い構造化レイアウトへ統一し、旧MarkdownベースのPDFを廃止する。
- スコープ: バックエンドのPDF生成処理、設定API、関連ドキュメント・テスト。CSV/Markdownエクスポートは後方互換を維持。

### 進捗チェックリスト
- [x] 構造化PDFレンダラの調整と患者情報/回答/サマリーのレイアウト検証
- [x] MarkdownベースPDF生成コードの削除と構造化レイアウトへの一本化
- [x] `/system/pdf-layout` 設定APIおよびトグル関連テストの削除
- [x] 管理ダウンロードAPI（単体/一括）が構造化PDFを常に生成するよう更新
- [x] CSV/Markdownエクスポート仕様の後方互換性確認
- [x] 運用ドキュメントを新仕様に更新（管理セットアップ手順書 など）

### 備考
- 既存利用者への影響は、構造化PDFが既定で生成される点のみ。旧PDF設定はDBに残存しても読み込まれない。
- 施設表示名の設定値は引き続きヘッダーに反映される。表示名未設定の場合は「Monshinクリニック」を利用。
- ReportLabの日本語フォント登録は `pdf_renderer` 側で一元管理しているため、追加の設定は不要。
