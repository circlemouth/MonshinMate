...
## 112. 問診結果PDFレイアウトの改善（2025-12-13）
- [x] PDF出力で患者情報ヘッダーを段組表示に変更し、問診票記入日にセッション確定日時を表示。
- [x] 選択肢回答の先頭から「・」を除去し、複数選択時はインデントで視認性を確保。
- [x] 自由記述回答の空欄は「未回答」として表示するよう `backend/app/main.py` を調整。
- [x] 出力整形ロジック向けのテストを `backend/tests/test_api.py` に追加。
- [x] バックエンド自動テスト実行: `cd backend && pytest`
