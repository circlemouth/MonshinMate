...
4. モデル名の扱いはプロバイダによって異なります。
   - `ollama` / `lm_studio` / `openai` では「使用可能なモデル一覧を取得」で候補を取得し、プルダウンから選択できます（入力中の `provider` や追加フィールドを `/llm/list-models` に送信）。
   - `gcp_vertex`（Google Cloud Vertex AI）は Google 側の制約で一覧取得ができないためボタン・プルダウンは表示されません。モデル名を直接入力し、「疎通テストを実行」で接続を確認してください。
